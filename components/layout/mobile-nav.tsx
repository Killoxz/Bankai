  ...
  return (
    <>
      <nav className="fixed inset-x-3 bottom-3 z-40 md:hidden">
        <div className="flex items-center justify-around rounded-2xl border border-white/10 bg-[#1c1c1c]/95 py-2 shadow-2xl backdrop-blur-md">
          {NAV_ITEMS.map((item) => {
            ...
            if (!item.href) {
              return (
                <button
                  ...
                  className="flex flex-1 flex-col items-center gap-1 py-1 text-white/50"
                >
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-3 border-b border-white/10 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        ...
        <button onClick={onClose} aria-label="Close search" className="shrink-0 text-white/60">
          <X className="size-5" />
        </button>
      </div>
